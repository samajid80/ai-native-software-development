...
```python
# Django-style (conceptual - showing the pattern)
# In real Django: from django.db import models
# For this example, we'll use a simplified placeholder

class models:
    """Placeholder to demonstrate Django pattern (requires django package)."""
    class Model:
        pass
    class CharField:
...
try:
    ...
    print("Pydantic example defined")
except ImportError:
    print("Pydantic not installed (optional dependency for this example)")
```
...
# (This would require defining UserProfileMeta, StringField, etc.)
...
# Note: Would require defining ValidationMeta
...
# Re-define request/response models for this block
@dataclass
class CreatePostRequest:
    title: str
    content: str
...
# Re-include all needed definitions for standalone execution
@dataclass
class CreatePostRequest:
    title: str
    content: str
...
class HandlerMeta(type):
    def __new__(mcs, name: str, bases: tuple, namespace: dict):
        cls = super().__new__(mcs, name, bases, namespace)
        if name != "BaseHandler":
            handler_name = name.replace("Handler", "").replace("_", "").lower()
            HandlerRegistry.handlers[handler_name] = cls
        return cls
...
class CreatePostHandler(BaseHandler):
    def process(self, request: CreatePostRequest) -> PostResponse:
        return PostResponse(
            id=42, title=request.title, content=request.content,
            tags=request.tags, author_id=1, created_at="2025-11-09T10:30:00Z"
        )
...
response = router.route("createpost", request)
